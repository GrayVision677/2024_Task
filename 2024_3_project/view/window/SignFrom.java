...
public class SignFrom extends BaseFrame{
	...
	public void setDsign() {
		...
		jpC.jpR.add(jbGet = new BaseButton("인증번호 받기").setcolor());
		jpC.jpR.add(jbYes = new BaseButton("확인").setcolor());
		...
		jpB.jpB.setFlowCenter().add(jbsign = new BaseButton("회원가입").setcolor());
	...
	public void setEvent() {
		...
		jbsign.addActionListener(e -> {
			...
			if(name.isBlank() || id.isBlank() || pw.isBlank() || Phone.length + "" == null || jtA.equals("본인확인 질문 답변") || jtA.getText().trim().isBlank()) {
				Msg.error("빈칸이 있습니다.");
				return;
			}
